...
pub async fn set_map_locally(
    ...
    let mut char: Character =
        assembly::character::assemble::assemble_char_by_id(&locked_state.db.clone(), char_id)
            .await?;
    char.model.map_wz = map_wz;
    db::character::setters::update_characters(&locked_state.db.clone(), vec![char.model]).await?;
    ...
    match map_scope {
        MapScope::SameChannelSameWorld => {
            ...
            for s in sessions {
                ...
                let mut char: Character = assembly::character::assemble::assemble_char_by_id(
                    &locked_state.db.clone(),
                    char_id,
                )
                .await?;
                char.model.map_wz = map_wz;
                db::character::setters::update_characters(
                    &locked_state.db.clone(),
                    vec![char.model],
                )
                .await?;
    ...
    for s in sessions {
        ...
        let mut char: Character =
            assembly::character::assemble::assemble_char_by_id(&locked_state.db.clone(), char_id)
                .await?;
        char.model.map_wz = map_wz;
        db::character::setters::update_characters(&locked_state.db.clone(), vec![char.model])
            .await?;
    }
    Ok(())
}
