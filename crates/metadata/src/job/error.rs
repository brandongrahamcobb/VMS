...
pub enum JobMetadataError {
    #[error("No skills found error in job metadata layer: {0}")]
    NoSkill(i16),

    #[error("Metadata service error in job metadata layer")]
    MetadataServiceError(#[from] MetadataServiceError),

    #[error("Parse integer error in job metadata layer")]
