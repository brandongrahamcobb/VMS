...
use crate::models::character::keybinding::model::{Keybinding, NewKeybindingInsert};
use crate::models::character::model::{Character, NewInsert};
...
impl CreateCharStore {
    pub async fn store_create_char(
        ...
        let char_insert = NewInsert::default(
            acc.model.id,
            reader.ign.clone(),
            reader.job_id,
            reader.face_id,
            reader.hair_id,
            reader.hair_color_id,
            reader.skin_id,
            reader.gender_id,
            map.model.id,
            world.model.id,
        ...
        let binds_insert: Vec<NewKeybindingInsert> =
            izip!(DEFAULT_KEY, DEFAULT_TYPE, DEFAULT_ACTION)
                .map(|(key, bind_type, action): (i32, i16, i32)| {
                    NewKeybindingInsert::default(char_model.id, key, bind_type, action)
                })
                .collect();
        ...
        let weapon_id = weapon_model.id;
        let regular_equips_insert = character::service::new_character_regular_equips_defaults(
            char_model.id,
            top_id,
            bottom_id,
            shoes_id,
        ...
        let cash_equips_insert =
            character::service::new_character_cash_equips_defaults(char_model.id)?;
        let pet_equips_insert =
            character::service::new_character_pet_equips_defaults(char_model.id)?;
        let android_equips_insert =
            character::service::new_character_android_equips_defaults(char_model.id)?;
