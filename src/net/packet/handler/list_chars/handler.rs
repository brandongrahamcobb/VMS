    ...
    pub async fn handle(
        ...
        let read = list_chars::read::read_list_chars_packet(packet)?;
        {
            ...
            state.sessions.update(session.id, |s| {
                s.channel_id = Some(read.channel_id.clone());
                s.world_id = Some(read.world_id.clone());
            });
        }
        ...
        let char_max =
            world::query::get_character_max_by_account_and_world_id(state, &acc_id, &read.world_id)
                .await
        ...
        let result =
            complete_list_chars_handler(&state, &read.channel_id, &chars, &char_max, &pic_status)
                .await?;
        Ok(result)
    }
}
...
async fn complete_list_chars_handler(
    ...
    let packet: Packet = Packet::new_empty()
        .build_list_chars_handler_packet(state, channel_id, chars, char_max, pic_status)
        .await?
        .finish();
