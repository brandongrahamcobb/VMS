...
impl Packet {
    pub async fn build_list_chars_handler_packet(
        ...
        self.write_byte(*channel_id as u8).map_err(WriteError)?;
        ...
        for char in chars {
            ...
            .await?;
            let cash_equips = equipment_set::query::get_cash_equipment_set_by_character_id(
                state.clone(),
                &char.id,
            )
            .await?;
        ...
        self.write_byte(*pic_status as u8) // use pic?
            .map_err(WriteError)?;
        self.write_int(*char_max as i32) // Number of character slots
            .map_err(WriteError)?;
        Ok(self)
    }
