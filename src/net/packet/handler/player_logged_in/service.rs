...
pub async fn create_equips_on_join(
    ...
    let regular_equips =
        equipment_set::query::get_regular_equipment_set_by_character_id(state.clone(), &char.id)
            .await?;
    let cash_equips =
        equipment_set::query::get_cash_equipment_set_by_character_id(state.clone(), &char.id)
            .await?;
...
pub async fn create_keybindings_on_join(
    ...
    let binds = keybinding::query::get_keybindings_by_character_id(state.clone(), &char.id).await?;
    let binds: Vec<Keybinding> =
        keybinding::service::normalize_keybindings(binds.clone(), &char.id);
    Ok(binds)
}
