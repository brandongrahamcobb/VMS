...
impl Packet {
    pub fn build_player_logged_in_handler_keymap_packet(
        ...
        for bind in binds {
            self.write_byte(bind.bind_type as u8).map_err(WriteError)?;
            self.write_int(bind.action as i32).map_err(WriteError)?;
        }
        Ok(self)
    }
    ...
    fn build_inventory_regular_equip_meta_part_packet(
        ...
        self.write_byte(false as u8).map_err(WriteError)?;
        self.write_long(0).map_err(WriteError)?;
        self.write_byte(0).map_err(WriteError)?;
        self.write_byte(0).map_err(WriteError)?;
        ...
        self.write_int(0).map_err(WriteError)?;
        self.write_long(0).map_err(WriteError)?;
        self.write_bytes(&[0u8; 12]).map_err(WriteError)?;
        Ok(self)
    }
