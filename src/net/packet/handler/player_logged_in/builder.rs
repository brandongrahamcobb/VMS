...
impl Packet {
    ...
    pub fn build_inventory_regular_equipment_part_packet(
        ...
        if !char.regular_equips.hat.model.id.is_negative() {
            self.build_inventory_regular_hat_part_packet(char.regular_equips.hat.clone())?;
        }
        if !char.regular_equips.face_acc.model.id.is_negative() {
            self.build_inventory_regular_face_acc_part_packet(
                char.regular_equips.face_acc.clone(),
            )?;
        }
        if !char.regular_equips.eye_acc.model.id.is_negative() {
            self.build_inventory_regular_eye_acc_part_packet(char.regular_equips.eye_acc.clone())?;
        }
        if !char.regular_equips.ear_acc.model.id.is_negative() {
            self.build_inventory_regular_ear_acc_part_packet(char.regular_equips.ear_acc.clone())?;
        }
        if !char.regular_equips.top.model.id.is_negative() {
            self.build_inventory_regular_top_part_packet(char.regular_equips.top.clone())?;
        }
        if !char.regular_equips.bottom.model.id.is_negative() {
            self.build_inventory_regular_bottom_part_packet(char.regular_equips.bottom.clone())?;
        }
        if !char.regular_equips.shoes.model.id.is_negative() {
            self.build_inventory_regular_shoes_part_packet(char.regular_equips.shoes.clone())?;
        }
        if !char.regular_equips.gloves.model.id.is_negative() {
            self.build_inventory_regular_gloves_part_packet(char.regular_equips.gloves.clone())?;
        }
        if !char.regular_equips.cape.model.id.is_negative() {
            self.build_inventory_regular_cape_part_packet(char.regular_equips.cape.clone())?;
        }
        if !char.regular_equips.shield.model.id.is_negative() {
            self.build_inventory_regular_shield_part_packet(char.regular_equips.shield.clone())?;
        }
        if !char.regular_equips.weapon.model.id.is_negative() {
            self.build_inventory_regular_weapon_part_packet(char.regular_equips.weapon.clone())?;
        }
        if !char.regular_equips.ring_one.model.id.is_negative() {
            self.build_inventory_regular_ring_one_part_packet(
                char.regular_equips.ring_one.clone(),
            )?;
        }
        if !char.regular_equips.ring_two.model.id.is_negative() {
            self.build_inventory_regular_ring_two_part_packet(
                char.regular_equips.ring_two.clone(),
            )?;
        }
        if !char.regular_equips.ring_three.model.id.is_negative() {
            self.build_inventory_regular_ring_three_part_packet(
                char.regular_equips.ring_three.clone(),
            )?;
        }
        if !char.regular_equips.ring_four.model.id.is_negative() {
            self.build_inventory_regular_ring_four_part_packet(
                char.regular_equips.ring_four.clone(),
            )?;
        }
        if !char.regular_equips.pendant_one.model.id.is_negative() {
            self.build_inventory_regular_pendant_one_part_packet(
                char.regular_equips.pendant_one.clone(),
            )?;
        }
        if !char.regular_equips.tamed_mob.model.id.is_negative() {
            self.build_inventory_regular_tamed_mob_part_packet(
                char.regular_equips.tamed_mob.clone(),
            )?;
        }
        if !char.regular_equips.saddle.model.id.is_negative() {
            self.build_inventory_regular_saddle_part_packet(char.regular_equips.saddle.clone())?;
        }
        if !char.regular_equips.medal.model.id.is_negative() {
            self.build_inventory_regular_medal_part_packet(char.regular_equips.medal.clone())?;
        }
        if !char.regular_equips.belt.model.id.is_negative() {
            self.build_inventory_regular_belt_part_packet(char.regular_equips.belt.clone())?;
        }
        if !char.regular_equips.pocket.model.id.is_negative() {
            self.build_inventory_regular_pocket_part_packet(char.regular_equips.pocket.clone())?;
        }
        if !char.regular_equips.book.model.id.is_negative() {
            self.build_inventory_regular_book_part_packet(char.regular_equips.book.clone())?;
        }
        if !char.regular_equips.pendant_two.model.id.is_negative() {
            self.build_inventory_regular_pendant_two_part_packet(
                char.regular_equips.pendant_two.clone(),
            )?;
        }
        if !char.regular_equips.shoulder.model.id.is_negative() {
            self.build_inventory_regular_shoulder_part_packet(
                char.regular_equips.shoulder.clone(),
            )?;
        }
        if !char.regular_equips.android.model.id.is_negative() {
            self.build_inventory_regular_android_part_packet(char.regular_equips.android.clone())?;
        }
        if !char.regular_equips.emblem.model.id.is_negative() {
            self.build_inventory_regular_emblem_part_packet(char.regular_equips.emblem.clone())?;
        }
        if !char.regular_equips.badge.model.id.is_negative() {
            self.build_inventory_regular_badge_part_packet(char.regular_equips.badge.clone())?;
        }
        if !char.regular_equips.subweapon.model.id.is_negative() {
            self.build_inventory_regular_subweapon_part_packet(
                char.regular_equips.subweapon.clone(),
            )?;
        }
        if !char.regular_equips.heart.model.id.is_negative() {
            self.build_inventory_regular_heart_part_packet(char.regular_equips.heart.clone())?;
        }
        Ok(self)
    }
    ...
    pub fn build_inventory_cash_equipment_part_packet(
        ...
        if !char.cash_equips.bottom.model.id.is_negative() {
            self.build_inventory_cash_bottom_part_packet(char.cash_equips.bottom.clone())?;
        }
        if !char.cash_equips.cape.model.id.is_negative() {
            self.build_inventory_cash_cape_part_packet(char.cash_equips.cape.clone())?;
        }
        if !char.cash_equips.ear_acc.model.id.is_negative() {
            self.build_inventory_cash_ear_acc_part_packet(char.cash_equips.ear_acc.clone())?;
        }
        if !char.cash_equips.eye_acc.model.id.is_negative() {
            self.build_inventory_cash_eye_acc_part_packet(char.cash_equips.eye_acc.clone())?;
        }
        if !char.cash_equips.face_acc.model.id.is_negative() {
            self.build_inventory_cash_face_acc_part_packet(char.cash_equips.face_acc.clone())?;
        }
        if !char.cash_equips.gloves.model.id.is_negative() {
            self.build_inventory_cash_gloves_part_packet(char.cash_equips.gloves.clone())?;
        }
        if !char.cash_equips.hat.model.id.is_negative() {
            self.build_inventory_cash_hat_part_packet(char.cash_equips.hat.clone())?;
        }
        if !char.cash_equips.ring_four.model.id.is_negative() {
            self.build_inventory_cash_ring_four_part_packet(char.cash_equips.ring_four.clone())?;
        }
        if !char.cash_equips.ring_one.model.id.is_negative() {
            self.build_inventory_cash_ring_one_part_packet(char.cash_equips.ring_one.clone())?;
        }
        if !char.cash_equips.ring_three.model.id.is_negative() {
            self.build_inventory_cash_ring_three_part_packet(char.cash_equips.ring_three.clone())?;
        }
        if !char.cash_equips.ring_two.model.id.is_negative() {
            self.build_inventory_cash_ring_two_part_packet(char.cash_equips.ring_two.clone())?;
        }
        if !char.cash_equips.shoes.model.id.is_negative() {
            self.build_inventory_cash_shoes_part_packet(char.cash_equips.shoes.clone())?;
        }
        if !char.cash_equips.top.model.id.is_negative() {
            self.build_inventory_cash_top_part_packet(char.cash_equips.top.clone())?;
        }
        if !char.cash_equips.weapon.model.id.is_negative() {
            self.build_inventory_cash_weapon_part_packet(char.cash_equips.weapon.clone())?;
        }
        Ok(self)
    }
