/* equip_stats/service.rs
 * The purpose of this module is to provide assisting functions for equipment statistics.
 *
...
pub async fn deserialize(item_wz: i32) -> Result<EquipStatsModel, ModelError> {
    ...
    Ok(equip_stats_model)
}

pub async fn get_equip_stats_by_item_id(
    state: &SharedState,
    id: i32,
) -> Result<EquipStats, ModelError> {
    let equip_stats_model =
        equip_stats::query::getters::get_equip_stats_model_by_id(state, id).await?;
    Ok(equip_stats_model.load()?)
}
